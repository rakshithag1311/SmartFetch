...
import { LogOut, Package, RefreshCw, Clock, Plus, Store, ChefHat, CheckCircle, XCircle, AlertCircle, TrendingUp, Trash2, Pencil, Save, X, ListOrdered, UtensilsCrossed } from 'lucide-react';
...
const ShopkeeperDashboard = () => {
  ...
  const toggleAvailable = async (item: ShopItem) => {
    const { error } = await supabase
      .from('shop_items')
  ...
  return (
    <div className="page-container fade-in">
      ...
      <div className="content-container">
        ...
        {/* Tabs */}
        <div className="flex gap-2 mb-5 p-1 bg-secondary rounded-2xl">
          <button
            onClick={() => setActiveTab('orders')}
            className={`flex-1 py-2 rounded-xl text-sm font-semibold flex items-center justify-center gap-1.5 transition-all ${
              activeTab === 'orders' ? 'bg-background shadow text-foreground' : 'text-muted-foreground'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-xl text-sm font-semibold flex items-center justify-center gap-1.5 transition-all ${
              activeTab === 'items' ? 'bg-background shadow text-foreground' : 'text-muted-foreground'
            }`}
          >
        ...
        {activeTab === 'items' ? (
          <div className="fade-in">
            {/* Add item form */}
            <form onSubmit={handleAddItem} className="card-base mb-5 space-y-3">
              <h3 className="text-sm font-bold text-foreground flex items-center gap-1.5">
                <Plus className="w-4 h-4 text-primary" /> Add New Item
              </h3>
              <input
                type="text"
                value={newItemName}
                onChange={e => setNewItemName(e.target.value)}
                placeholder="Item name (e.g. Veg Sandwich)"
                className="input-field"
                required
                maxLength={80}
              />
            ...
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base font-bold text-foreground">
                Your Menu
                {items.length > 0 && <span className="ml-2 text-xs text-muted-foreground font-normal">({items.length})</span>}
              </h2>
            </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="flex-1 py-2 text-sm rounded-2xl border-2 border-border font-semibold text-muted-foreground hover:bg-secondary transition-colors flex items-center justify-center gap-1.5"
                          >
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-primary font-bold text-sm">₹{Number(item.price).toFixed(0)}</span>
