        ...
        if (role === 'shopkeeper') {
          ...
          if (createNewShop) {
            ...
            const { error: shopErr } = await supabase.from('shops').insert({
              ...
              category: newShopCategory.trim() || 'General',
              description: newShopDescription.trim() || '',
            });
            if (shopErr) {
              toast.error(`Could not create shop: ${shopErr.message}`);
              setLoading(false);
              return;
            }
            shopIdToLink = newId;
          ...
          if (shopIdToLink) {
            await supabase.from('shop_staff').insert({ user_id: newUser.id, shop_id: shopIdToLink });
          }
        }
              ...
              <div className="flex gap-2 p-1.5 bg-secondary rounded-2xl">
                <button
                  ...
                  className={`flex-1 py-2 rounded-xl font-semibold text-xs transition-all ${
                    !createNewShop ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'
                  }`}
                >
                  Join existing shop
                </button>
                ...
                <>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Cafe, Bakery, Grocery"
